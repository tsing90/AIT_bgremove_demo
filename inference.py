...
def seg_process(img_t, img_m, re_size, net_t, net_m):
    ...
    trimap = trimap.cpu().data.numpy().astype(np.uint8)
...
def matting(img_path):
    ...
    tensor_img_m = torch.from_numpy(img_m.astype(np.float32)[np.newaxis, :, :, :]).permute(0, 3, 1, 2)
    ...
    print('time taken: {:.5f} s'.format(time.time()-start))
